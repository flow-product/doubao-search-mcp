...
const GET_SEARCH_BROWSE_PLAN_TOOL: Tool = {
    ...
    switch (name) {
      ...
      case "get_search_browse_plan": {
        ...
        if (!query) {
          throw new McpError(
            ErrorCode.InvalidParams,
            "User’s original query is required"
          );
        }
        ...
        try {
          ...
          const messageResponse = await fetch(
            `${COZE_API_CONFIG.BASE_URL}${COZE_API_CONFIG.ENDPOINTS.MESSAGE_LIST}?conversation_id=${conversationId}`,
            {
              method: "POST",
              headers: {
                "Authorization": `Bearer ${process.env.COZE_KEY}`,
